...
2. **Endpointing**: (Future Task) Voice Activity Detection (VAD) identifies active speech chunks.
...
4. **Networking**: (Future Task) Transcripts are relayed via Bluetooth / Wi-Fi Direct.
5. **TTS Inference**: (Future Task) Received packets are synthesized natively via the local offline TTS model and piped to the system speaker.
